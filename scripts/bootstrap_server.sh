...
grep -q '^MINI_OPS_WEB_UPDATE_TIMEOUT_SECS=' "$DEPLOY_TARGET_DIR/.env" || echo 'MINI_OPS_WEB_UPDATE_TIMEOUT_SECS=1800' >> "$DEPLOY_TARGET_DIR/.env"
...
grep -q '^SECURITY_ALLOWED_PUBLIC_PORTS=' "$DEPLOY_TARGET_DIR/.env" || echo 'SECURITY_ALLOWED_PUBLIC_PORTS=' >> "$DEPLOY_TARGET_DIR/.env"
grep -q '^SECURITY_ALLOWED_LOOPBACK_PORTS=' "$DEPLOY_TARGET_DIR/.env" || echo 'SECURITY_ALLOWED_LOOPBACK_PORTS=' >> "$DEPLOY_TARGET_DIR/.env"
